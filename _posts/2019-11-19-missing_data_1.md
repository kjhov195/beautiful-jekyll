---
layout: post
title: Missing data(1)-Intuitive understanding
...
A 환자가 1월부터 12월까지 실험에 참여한다고 해보자. 이 환자가 만약 3월에 어떠한 이유로 혈압을 측정하지 않았지만 1월~2월, 4월~12월에는 병원에 방문하여 혈압을 측정하였다고 한다. 이렇게 중간에 결측치가 발생한 경우, 이 결측치를 __intermittent missing value__ 라고 한다.

반면 B환자는 어떤 이유에선지 1월부터 8월까지 매달 병원에 방문하여 혈압을 측정하였으나, 9월 이후로는 병원에 방문하지 않았다고 한다. 이러한 경우를 __중도탈락__, 혹은 __dropout__ 이라고 한다.
